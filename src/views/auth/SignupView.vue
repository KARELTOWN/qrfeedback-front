...
const { signup, verifyOtp, resendOtp: resendOtpRequest } = useAuth();
const form = ref<SignupInput>({ email: '', password: '' });
const otpForm = ref<OtpForm>({ code: '' });
const errors = ref<FormErrors<SignupInput>>({});
...
async function submit() {
  toast.value = '';
  errors.value = await validateForm(signupSchema, form.value);
...
async function submitOtp() {
  ...
  loading.value = true;
  try {
    await verifyOtp(form.value.email, otpForm.value.code, otpPurpose.value);
    router.push('/dashboard');
  } catch (err) {
    toast.value = err instanceof Error ? err.message : 'Erreur inconnue';
  } finally {
  ...
  <div class="min-h-screen bg-slate-100">
    ...
    <section class="w-full max-w-3xl rounded-3xl bg-white px-6 py-10 shadow-xl shadow-slate-200 sm:px-16 sm:py-14">
      ...
      <form v-if="!otpStep" class="mx-auto mt-10 max-w-xl space-y-5" @submit.prevent="submit">
        ...
        <label class="block">
          <PasswordField v-model="form.password" placeholder="Mot de passe fort" />
