...
<template>
  <section class="grid gap-5">
    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p class="mt-1 font-semibold text-slate-500">Entreprises ayant généré un QR, avec ou sans compte utilisateur.</p>
        </div>
    ...
    <div class="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm">
      <table class="min-w-[920px] w-full text-left"><thead class="bg-slate-50"><tr><th class="px-5 py-4 font-black">Entreprise</th><th class="px-5 py-4 font-black">Email</th><th class="px-5 py-4 font-black">Compte</th><th class="px-5 py-4 font-black">Date demande</th><th class="px-5 py-4 font-black">Action</th></tr></thead>
        <tbody>
          <tr v-for="company in qrRequests.companies" :key="company._id" class="border-t border-slate-200 transition hover:bg-slate-50">
          ...
          <tr v-if="!qrRequests.companies.length">
            <td colspan="5" class="px-5 py-10 text-center font-bold text-slate-500">Aucune demande QR ne correspond au filtre.</td>
          </tr>
        </tbody>
      </table>
    </div>
